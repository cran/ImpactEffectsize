...
plot2Densities <- function(Data,Cls,col=c("red","blue"), pde=TRUE, 
  ...
  if(hasArg(pde) == TRUE & pde == FALSE) {
    ...
    pdx <- ParetoDensityEstimationIE(Data)$kernels
    pd1 <- ParetoDensityEstimationIE(Data = Data[Cls==unique(Cls)[1]], kernels = pdx)$paretoDensity
    pd2 <- ParetoDensityEstimationIE(Data = Data[Cls==unique(Cls)[2]], kernels = pdx)$paretoDensity
  }
  xmin=min(pdx)
  xmax=max(pdx)
  ymax=max(pd1,pd2)
